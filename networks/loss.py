...
def cross_entropy_loss(
    ...
    return F.cross_entropy(input, target, weight, size_average, ignore_index, reduction=reduction)
...
def multi_smooth_loss(
    ...
    for i in range(0, len(input)):
        if i in [1, len(input)-1]:
            ...
            ymask = prob.data.new(prob.size()).zero_()
            ymask = ymask.scatter_(1, target.view(-1, 1), 1)
            ymask = smooth_ratio*ymask + \
                (1-smooth_ratio)*(1-ymask)/(len(input[i][1])-1)
            loss_tmp = - weight_loss[i]*((prob*ymask).sum(1).mean())
        else:
            loss_tmp = weight_loss[i]*F.cross_entropy(
                input[i], target, weight, size_average, ignore_index, reduction=reduction)
        loss += loss_tmp
