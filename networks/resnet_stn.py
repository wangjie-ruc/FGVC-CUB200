...
class STN_HEAD(nn.Module):
    def __init__(self, in_channels=512, size=8):
        super(STN_HEAD, self).__init__()
    ...
    def forward(self, feature):
        ...
        x = self.localization(feature)
        x = x.view(x.size(0), -1)
        theta = self.fc_loc(x)
        theta = torch.sigmoid(theta)
        theta = theta.unsqueeze(2)

        matrix = torch.zeros(x.size(0), 2, 2).to(x.device)
        matrix[:, 0, 0] = 0.5
        matrix[:, 1, 1] = 0.5
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
                 ...
                 norm_layer=None, with_stn=True, size=8):
        ...
        if with_stn:
            self.stn = STN_HEAD(self.inplanes, size=size)
...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    ...
    if pretrained:
        ...
        ) if k in model_dict and v.size() == model_dict[k].size()}, strict=False)
    return model
