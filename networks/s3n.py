...
def s3n(
    ...
    radius: float = 0.08, 
    radius_inv: float = 0.2,
    pretrained: bool = True) -> nn.Module:
    """ Selective sparse sampling.
    """

    classify_network = ft_resnet(mode=mode, fc_or_fcn = 'fc',num_classes=num_classes, pretrained=pretrained)
    # classify_network = resnet.__dict__[mode](num_classes=num_classes, pretrained=False)
